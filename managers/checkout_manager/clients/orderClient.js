...
const createOrder = async (token, email, deliveryAddress, totalMoney, items, couponId, loyaltyPoint) => {
    try {
        const orderResponse = await axios.post(`${ORDER_SERVICE_ROOT}/orders`, 
            {
                email: email, deliveryAddress: deliveryAddress, totalMoney: totalMoney,
