...
passport.use(new GoogleStrategy({
    ...
    try {
        ...
        const userAuth = await AuthProvider.findOne({ providerUserId: profile.id })
        ...
        if (!userAuth) {
            ...
            const userProvider = await AuthProvider.create({
                userId: user._id,
                providerName: 'google',
...
app.get('/oauth/google/callback', passport.authenticate('google', { session: false }), (req, res) => {
    const user = req.user
    const authToken = jwt.sign({ id: user._id, email: user.email, role: user.role }, JWT_LOGIN_SECRET, { expiresIn: '1h' })
...
app.post('/register', validateRegister, async (req, res) => {
    ...
    if (await User.findOne({ email, password: { $exists: true } })) { return res.status(400).json({error: "This email was used"}) }

    const password = generatePassword()
    const hashedPassword = await bcrypt.hash(password, 10)
    const expiredDate = new Date(Date.now() + EXPIRY * 60 * 1000)

    const user = new User({ email, password: hashedPassword, expiredDate })

    if (dryrun) { return res.json({ user }) }
...
app.get('/register/callback', async (req, res) => {
    ...
    await User.updateOne({ email, password: { $exists: true } }, { $unset: { expiredDate: "" } })
...
app.post('/reset', async (req, res) => {
    ...
    const resetToken = jwt.sign({ email: user.email }, JWT_RESET_SECRET, { expiresIn: '15m' })
...
app.put('/reset', async (req, res) => {
    ...
    try {
        ...
        const hashedPassword = await bcrypt.hash(password, 10)
        const user = await User.findOne({ email: payload.email, password: { $exists: true } })
        ...
        await User.updateOne({ email: user.email, password: { $exists: true } }, { password: hashedPassword })
